# ENGINE_POKEMON.md — Engine Notes from 45 Sprints of Pokemon Development

## Overview

Pokemon Gold/Silver/Crystal recreation built on the Crusty engine. ~12,500 lines of Rust across 5 files, implementing ~63 species, 33 maps (Johto through Olivine City), turn-based battles with stat stages, and a full overworld with trainer battles. All rendering is Rust-side framebuffer at GBC resolution (160x144), with a JavaScript overlay canvas for battle sprites loaded from the web. Scope: Johto region only (no Kanto post-game).
...
**Never-Miss Moves (Sprint 45)**
Accuracy sentinel: moves with `accuracy >= 255` bypass the accuracy/evasion check entirely. Used for Faint Attack (and future Swift). Clean one-line guard in both player and enemy accuracy check paths.

### Pain Points & Lessons

**File Size**
`mod.rs` hit ~3450 lines and `maps.rs` ~5300 lines. Both are well past comfortable navigation. maps.rs especially — each new city adds ~300-400 lines of tile/collision/warp data. A future refactor could split map data into per-region files or generate it from a tilemap editor.
...
10. **Never-miss sentinel (Sprint 45)**: `accuracy >= 255` means the move always hits regardless of accuracy/evasion modifiers. Matches Gen 2 internal convention for moves like Faint Attack and Swift.

## File Layout

| File | Lines | Purpose |
|------|-------|---------|
| `mod.rs` | ~3450 | Game logic: phases, battle, menus, overworld, camera, stat stages |
| `maps.rs` | ~5300 | 33 maps with tiles, collision, NPCs, warps, encounters |
| `sprites.rs` | 1475 | Tile sprite data (indexed palette, 16x16 tiles) |
| `data.rs` | ~1100 | ~63 species, ~80 moves, item DB, type chart, damage calc |
| `render.rs` | 608 | Rendering helpers: tiles, HP bars, menu boxes, text |

## Species Implemented (~63)

**Starters**: Chikorita/Bayleef, Cyndaquil/Quilava, Totodile/Croconaw
**Early routes**: Pidgey/Pidgeotto, Rattata/Raticate, Sentret, Hoothoot/Noctowl, Caterpie, Weedle, Ledyba, Spinarak, Mareep/Flaaffy
**Caves/special**: Geodude, Zubat, Bellsprout, Gastly/Haunter/Gengar, Onix, Magikarp, Wooper, Hoppip/Skiploom
**Gym Pokemon**: Metapod/Butterfree, Kakuna/Beedrill, Scyther (Bugsy), Miltank/Clefairy (Whitney), Magnemite/Steelix (Jasmine)
...
**Route 38-39**: Farfetch'd, Tauros, Doduo, Psyduck, Slowpoke, Pikachu, Poliwhirl/Poliwag, Krabby, Mr. Mime, Corsola
**Olivine**: Spearow/Fearow, Machop, Marill, Ampharos
...
**Ecruteak → Olivine**: Route38, Route39, OlivineCity, OlivineGym, OlivineLighthouse

## Badges Implemented (5/8)
...
| Mineral Badge | OlivineGym | Jasmine | Steel |
...
**Sprint 45**: All 33 warps across Route38/39/OlivineCity/OlivineGym/OlivineLighthouse verified correct. Fixed Faint Attack accuracy (100→255 sentinel for never-miss). All move categories and species base stats verified correct for Gen 2.
