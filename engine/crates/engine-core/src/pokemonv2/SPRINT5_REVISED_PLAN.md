# Sprint 5 Revised Implementation Plan: Route 31 + Route 31/Violet Gate + Violet City Exterior

> Frodo's phase-by-phase plan derived from Gandalf's SPRINT5_ARCHITECTURE.md.
> Revised by Bilbo with 2 modifications incorporated (marked with **[BILBO MOD]**).
...
### **[BILBO MOD 1]** 2I. Delete Existing `test_route31_stub` Test

Delete the existing `test_route31_stub` test (maps.rs ~line 1797):

```rust
    // DELETE THIS TEST -- superseded by test_route31_dimensions and test_route31_has_encounters
    // #[test]
...
### **[BILBO MOD 2]** 4C. Update `test_all_map_entry_scripts_dont_panic` in mod.rs

Find the existing `test_all_map_entry_scripts_dont_panic` test (~line 941) and update the `all_maps` array to include Sprint 4 and Sprint 5 maps:
...
| `maps.rs` | ~500 | 12 new MapId variants, build_route31() full implementation, build_route31_violet_gate(), build_violet_city(), build_violet_city_stub() x7, build_dark_cave_stub(), build_connection_stub() x2, build_route31_encounters(), load_map() dispatcher, **[BILBO MOD 1] delete test_route31_stub** |
...
| `mod.rs` | ~30 | VioletCity callback, warp_to_last_pokecenter update, **[BILBO MOD 2] update test_all_map_entry_scripts_dont_panic** |
| **Total** | **~810** | |
