...
const MODE_STARTER_CHOICE: f64 = 10.0;
const MODE_BATTLE_ITEMS: f64 = 11.0;
const MODE_BATTLE_SWITCH: f64 = 12.0;
const MODE_INTERIOR: f64 = 13.0;
...
const BPHASE_CATCH_AIM: f64 = 4.0;
const BPHASE_CATCH_SHOT: f64 = 5.0;
...
// ═══════════════════════════════════════════════════════════════════════
// ATTACK TYPES — Each monster has 2-4 attacks with unique minigolf physics
// ═══════════════════════════════════════════════════════════════════════

#[derive(Clone, Copy, PartialEq, Debug)]
enum AttackKind {
    /// Standard orb — no special physics
    Strike,
    /// Power Shot — 1.5x velocity, 0.7x friction (heavy ball)
    PowerShot,
    /// Curve Ball — orb curves left/right during flight
    CurveBall,
    /// Multi Strike — fires 3 smaller orbs in a spread pattern
    MultiStrike,
    /// Piercing Shot — ignores bumpers, passes through
    PiercingShot,
    /// Homing Orb — slightly steers toward target each frame
    HomingOrb,
    /// Ricochet — extra bouncy off walls (1.0 damping instead of 0.8)
    Ricochet,
    /// Frost Shot — slows to a crawl near target, increases precision
    FrostShot,
    /// Thunder Bolt — instant hit, no physics, but lower damage
    ThunderBolt,
    /// Shadow Sneak — invisible orb until near target
    ShadowSneak,
}
...
struct Attack {
    ...
    kind: AttackKind,
    element: Element,
    power: f64,        // damage multiplier (1.0 = normal)
    description: &'static str,
}

/// Get attacks available for a species based on its element and level
fn get_attacks(species_id: u8, level: f64) -> Vec<Attack> {
    let sp = get_species(species_id);
    let elem = sp.element;
    let mut attacks = vec![
        // Every monster gets basic Strike
        Attack { name: "Strike", kind: AttackKind::Strike, element: elem, power: 1.0,
                 description: "Standard energy orb" },
    ];

    // Element-specific attack at level 1
    match elem {
        Element::Fire => attacks.push(Attack {
            name: "Flame Shot", kind: AttackKind::PowerShot, element: Element::Fire,
            power: 1.3, description: "Blazing fast heavy orb",
        }),
        Element::Water => attacks.push(Attack {
            name: "Tidal Curve", kind: AttackKind::CurveBall, element: Element::Water,
            power: 1.1, description: "Orb curves around obstacles",
        }),
        Element::Leaf => attacks.push(Attack {
            name: "Vine Whip", kind: AttackKind::HomingOrb, element: Element::Leaf,
            power: 0.9, description: "Gently steers toward target",
        }),
        Element::Electric => attacks.push(Attack {
            name: "Thunderbolt", kind: AttackKind::ThunderBolt, element: Element::Electric,
            power: 0.8, description: "Instant hit, no aiming needed",
        }),
        Element::Earth => attacks.push(Attack {
            name: "Boulder Toss", kind: AttackKind::PowerShot, element: Element::Earth,
            power: 1.4, description: "Slow but devastating",
        }),
        Element::Ice => attacks.push(Attack {
            name: "Frost Beam", kind: AttackKind::FrostShot, element: Element::Ice,
            power: 1.0, description: "Slows near target for precision",
        }),
        Element::Shadow => attacks.push(Attack {
            name: "Shadow Sneak", kind: AttackKind::ShadowSneak, element: Element::Shadow,
            power: 1.2, description: "Invisible until near target",
        }),
        Element::Light => attacks.push(Attack {
            name: "Prism Scatter", kind: AttackKind::MultiStrike, element: Element::Light,
            power: 0.6, description: "3 orbs in a spread",
        }),
        Element::Normal => attacks.push(Attack {
            name: "Quick Shot", kind: AttackKind::Ricochet, element: Element::Normal,
            power: 0.9, description: "Extra bouncy off walls",
        }),
    }

    // Level 10 unlock: secondary attack
    if level >= 10.0 {
        match elem {
            Element::Fire => attacks.push(Attack {
                name: "Inferno Spread", kind: AttackKind::MultiStrike, element: Element::Fire,
                power: 0.5, description: "3 fireballs in a fan",
            }),
            Element::Water => attacks.push(Attack {
                name: "Hydro Pierce", kind: AttackKind::PiercingShot, element: Element::Water,
                power: 1.2, description: "Passes through obstacles",
            }),
            Element::Leaf => attacks.push(Attack {
                name: "Razor Leaf", kind: AttackKind::MultiStrike, element: Element::Leaf,
                power: 0.5, description: "3 leaf blades in a spread",
            }),
            Element::Electric => attacks.push(Attack {
                name: "Spark Bounce", kind: AttackKind::Ricochet, element: Element::Electric,
                power: 1.1, description: "Bounces off every surface",
            }),
            Element::Earth => attacks.push(Attack {
                name: "Tremor", kind: AttackKind::ThunderBolt, element: Element::Earth,
                power: 0.7, description: "Instant ground shockwave",
            }),
            Element::Ice => attacks.push(Attack {
                name: "Blizzard", kind: AttackKind::MultiStrike, element: Element::Ice,
                power: 0.5, description: "Icy shard barrage",
            }),
            Element::Shadow => attacks.push(Attack {
                name: "Dark Pulse", kind: AttackKind::PiercingShot, element: Element::Shadow,
                power: 1.1, description: "Pierces all barriers",
            }),
            Element::Light => attacks.push(Attack {
                name: "Holy Beam", kind: AttackKind::PiercingShot, element: Element::Light,
                power: 1.3, description: "Piercing light beam",
            }),
            Element::Normal => attacks.push(Attack {
                name: "Headbutt", kind: AttackKind::PowerShot, element: Element::Normal,
                power: 1.3, description: "Powerful direct hit",
            }),
        }
    }

    // Level 20 unlock: ultimate
    if level >= 20.0 {
        match elem {
            Element::Fire => attacks.push(Attack {
                name: "Solar Flare", kind: AttackKind::HomingOrb, element: Element::Fire,
                power: 1.5, description: "Homing fireball",
            }),
            Element::Water => attacks.push(Attack {
                name: "Tsunami", kind: AttackKind::HomingOrb, element: Element::Water,
                power: 1.4, description: "Unavoidable wave",
            }),
            Element::Leaf => attacks.push(Attack {
                name: "Forest Wrath", kind: AttackKind::PiercingShot, element: Element::Leaf,
                power: 1.5, description: "Piercing vine spear",
            }),
            Element::Electric => attacks.push(Attack {
                name: "Lightning Storm", kind: AttackKind::MultiStrike, element: Element::Electric,
                power: 0.7, description: "3 thunderbolts at once",
            }),
            _ => attacks.push(Attack {
                name: "Ultimate", kind: AttackKind::HomingOrb, element: elem,
                power: 1.4, description: "Powerful homing attack",
            }),
        }
    }
...
struct RivalTrainer {
    ...
    tile_x: usize,
    tile_y: usize,
    team: &'static [(u8, u8)], // (species_id, level)
    pre_battle_dialogue: &'static [&'static str],
    post_battle_dialogue: &'static [&'static str],
    body_color: Color,
    hair_color: Color,
}

fn get_rivals() -> Vec<RivalTrainer> {
    vec![
        RivalTrainer {
            name: "Rival Kai",
            zone: Zone::VerdantPath,
            tile_x: 15, tile_y: 16,
            team: &[(2, 7), (17, 6)], // Emberpup + Scurratt
            pre_battle_dialogue: &[
                "Hey! You're the new",
                "trainer from Pebble Town?",
                "I'm Kai! Let's see",
                "what you've got!",
            ],
            post_battle_dialogue: &[
                "Not bad! You might",
                "actually have what it",
                "takes. See you around!",
            ],
            body_color: Color { r: 220, g: 80, b: 80, a: 255 },
            hair_color: Color { r: 40, g: 30, b: 60, a: 255 },
        },
        RivalTrainer {
            name: "Trainer Luna",
            zone: Zone::EmberHollow,
            tile_x: 10, tile_y: 16,
            team: &[(4, 11), (6, 10), (16, 9)], // Bubblefin + Zapkit + Fluffmole
            pre_battle_dialogue: &[
                "The heat here doesn't",
                "bother me one bit!",
                "I'm Luna, and I'll",
                "show you real strategy!",
            ],
            post_battle_dialogue: &[
                "Impressive! You use",
                "type matchups well.",
                "The Guardian won't be",
                "so easy though!",
            ],
            body_color: Color { r: 100, g: 80, b: 200, a: 255 },
            hair_color: Color { r: 240, g: 200, b: 80, a: 255 },
        },
        RivalTrainer {
            name: "Rival Kai",
            zone: Zone::CoralShore,
            tile_x: 14, tile_y: 25,
            team: &[(3, 16), (8, 14), (17, 13)], // Blazewolf + Pebblet + Scurratt
            pre_battle_dialogue: &[
                "We meet again! My team",
        ...
        RivalTrainer {
            ...
            zone: Zone::DeepCave,
            tile_x: 14, tile_y: 20,
            team: &[(10, 18), (12, 17), (8, 19)], // Frostkit + Shadewisp + Pebblet
            pre_battle_dialogue: &[
                "Few trainers venture",
                "into these caves...",
                "I've trained here for",
                "years. Show me your skill!",
            ],
            post_battle_dialogue: &[
            ...
            body_color: Color { r: 60, g: 140, b: 100, a: 255 },
            hair_color: Color { r: 200, g: 200, b: 210, a: 255 },
        },
        RivalTrainer {
            name: "Rival Kai",
            zone: Zone::ShadowVale,
            tile_x: 15, tile_y: 12,
            team: &[(3, 24), (7, 22), (9, 23), (17, 21)],
            pre_battle_dialogue: &[
                "This is it! Our final",
                "battle before the Spire!",
                "I won't hold back!",
            ],
            post_battle_dialogue: &[
                "You really are the",
...
// Catch animation (physics-based: slingshot the orb AT the monster)
const K_CATCH_TIMER: &str = "catch_timer";
const K_CATCH_SUCCESS: &str = "catch_success";

// Attack selection
const K_SELECTED_ATTACK: &str = "sel_attack"; // index into attack list
const K_NUM_ATTACKS: &str = "num_attacks";     // count of available attacks
const K_ATTACK_KIND: &str = "atk_kind";        // current attack's AttackKind as f64

// Multi-strike sub-orbs (for MultiStrike attacks)
const K_MULTI_COUNT: &str = "multi_count";     // orbs remaining
const K_MULTI_BALL2_X: &str = "mb2_x";
const K_MULTI_BALL2_Y: &str = "mb2_y";
const K_MULTI_BALL2_VX: &str = "mb2_vx";
const K_MULTI_BALL2_VY: &str = "mb2_vy";
const K_MULTI_BALL3_X: &str = "mb3_x";
const K_MULTI_BALL3_Y: &str = "mb3_y";
const K_MULTI_BALL3_VX: &str = "mb3_vx";
const K_MULTI_BALL3_VY: &str = "mb3_vy";

// Starter choice
const K_STARTER_SEL: &str = "starter_sel"; // 0=Sproutail, 1=Emberpup, 2=Bubblefin
const K_STARTER_CONFIRMED: &str = "starter_confirmed";

// Rival trainers
const K_RIVAL_IDX: &str = "rival_idx";  // which rival we're fighting
const K_RIVAL_DEFEATED: &str = "rivals_beaten"; // bitfield of defeated rivals

// Battle sub-menu
const K_BATTLE_SUB: &str = "battle_sub";  // 0=main, 1=attacks, 2=items, 3=switch
const K_ITEM_SEL: &str = "b_item_sel";    // selected item in battle items menu
...
fn get_dialogue(id: u32) -> &'static [&'static str] {
    match id {
        ...
        // Rival Kai first encounter
        21 => &[
            "Hey! You're a new trainer?",
            "I'm Kai, and I'm going to",
            "be the S-League Champion!",
        ...
        // Rival Kai second encounter
        23 => &[
            "We meet again! I've been",
            "training hard since last",
            "time. Ready for round two?",
        ],
        // Trainer Luna encounter
        24 => &[
            "The flames of Ember Hollow",
            "called me here. I'm Luna,",
            "keeper of fire spirits.",
            "Show me your strength!",
        ],
        // Trainer Luna defeated
        25 => &[
            "Your bond with your",
            "creatures is impressive.",
            "The fire spirits approve!",
        ],
        // Trainer Sage encounter
        26 => &[
            "Few venture this deep...",
            "I am Sage, student of the",
            "ancient crystal spirits.",
            "Prove your worth!",
        ],
        // Trainer Sage defeated
        27 => &[
            "The crystals have chosen.",
            "You are worthy of the",
            "deepest mysteries.",
        ],
        // Professor starter choice prompt
        28 => &[
            "Welcome, young trainer!",
            "I'm Professor Pebble.",
            "Today you begin your",
            "Spirit Creature journey!",
            "Choose your first partner!",
        ],
        // No spirit orbs message
        29 => &[
            "You don't have any Spirit",
            "Orbs! Buy some at a shop.",
        ],
        _ => &["..."],
    }
}
...
pub fn on_pointer_down(engine: &mut Engine, x: f64, y: f64) {
    ...
    if mode == MODE_BATTLE {
        ...
        let sub = gs(engine, K_BATTLE_SUB);

        // Attack selection sub-menu
        if sub == 1.0 && bphase == BPHASE_PLAYER_AIM {
            handle_attack_select(engine, x, y);
            return;
        }

        if (bphase == BPHASE_PLAYER_AIM || bphase == BPHASE_CATCH_AIM)
            && gs(engine, K_BALL_ACTIVE) == 0.0
        {
            // Battle HUD buttons (bottom 120px)
            if y > HEIGHT - 120.0 {
                let btn_w = WIDTH / 4.0;
                if x < btn_w {
                    // ATTACK — open attack type selector
                    play_ui_click(engine);
                    ss(engine, K_BATTLE_SUB, 1.0);
                } else if x < btn_w * 2.0 {
                    // ITEMS — open battle item menu
                    play_ui_click(engine);
                    ss(engine, K_MODE, MODE_BATTLE_ITEMS);
                    ss(engine, K_ITEM_SEL, 0.0);
                } else if x < btn_w * 3.0 {
                    // CATCH — start catch aiming mode (physics-based)
                    try_catch_aim(engine);
                } else {
                    // RUN
                    try_run(engine);
                }
            ...
            // Start aiming in the arena area (using selected attack)
    ...
    if mode == MODE_BATTLE_ITEMS {
        handle_battle_items_tap(engine, x, y);
        return;
    }

    if mode == MODE_BATTLE_SWITCH {
        handle_battle_switch_tap(engine, x, y);
        return;
    }

    if mode == MODE_STARTER_CHOICE {
        handle_starter_choice_tap(engine, x, y);
        return;
    }
...
pub fn on_pointer_up(engine: &mut Engine, x: f64, y: f64) {
    ...
    if mode == MODE_BATTLE && gs(engine, K_AIMING) == 1.0 {
        ...
        if dist > 15.0 {
            let atk_kind = f64_to_attack_kind(gs(engine, K_ATTACK_KIND));

            // Power varies by attack type
            let power_mult = match atk_kind {
                AttackKind::PowerShot => 1.5,
                AttackKind::FrostShot => 0.8,
                AttackKind::ShadowSneak => 0.9,
                AttackKind::MultiStrike => 0.7,
                _ => 1.0,
            };

            let power = (dist * 12.0 * power_mult).min(2200.0);
            let angle = dy.atan2(dx);

            if atk_kind == AttackKind::MultiStrike {
                // Fire 3 orbs in a spread
                let spread = 0.15; // radians spread
                for i in 0..3 {
                    let a = angle + (i as f64 - 1.0) * spread;
                    let p = power * 0.8;
                    match i {
                        0 => {
                            ss(engine, K_BALL_VX, a.cos() * p);
                            ss(engine, K_BALL_VY, a.sin() * p);
                        }
                        1 => {
                            ss(engine, K_MULTI_BALL2_X, gs(engine, K_BALL_X));
                            ss(engine, K_MULTI_BALL2_Y, gs(engine, K_BALL_Y));
                            ss(engine, K_MULTI_BALL2_VX, a.cos() * p);
                            ss(engine, K_MULTI_BALL2_VY, a.sin() * p);
                        }
                        _ => {
                            ss(engine, K_MULTI_BALL3_X, gs(engine, K_BALL_X));
                            ss(engine, K_MULTI_BALL3_Y, gs(engine, K_BALL_Y));
                            ss(engine, K_MULTI_BALL3_VX, a.cos() * p);
                            ss(engine, K_MULTI_BALL3_VY, a.sin() * p);
                        }
                    }
                }
                ss(engine, K_MULTI_COUNT, 3.0);
            } else {
            ...
            if bphase == BPHASE_CATCH_AIM {
                ss(engine, K_BPHASE, BPHASE_CATCH_SHOT);
            } else {
...
fn handle_attack_select(engine: &mut Engine, _x: f64, y: f64) {
    let active = gs(engine, K_ACTIVE_MON) as usize;
    let species_id = gs_team(engine, active, "species") as u8;
    let level = gs_team(engine, active, "level");
    let attacks = get_attacks(species_id, level);

    let start_y = 300.0;
    let item_h = 55.0;

    for (i, _atk) in attacks.iter().enumerate() {
        let iy = start_y + i as f64 * item_h;
        if y >= iy && y < iy + item_h {
            ss(engine, K_SELECTED_ATTACK, i as f64);
            ss(engine, K_ATTACK_KIND, attack_kind_to_f64(attacks[i].kind));
            ss(engine, K_BATTLE_SUB, 0.0);
            play_ui_click(engine);

            // ThunderBolt is instant — no aiming needed
            if attacks[i].kind == AttackKind::ThunderBolt {
                do_instant_attack(engine, &attacks[i]);
            }
            return;
        }
    }
...
fn attack_kind_to_f64(kind: AttackKind) -> f64 {
    match kind {
        AttackKind::Strike => 0.0,
        AttackKind::PowerShot => 1.0,
        AttackKind::CurveBall => 2.0,
        AttackKind::MultiStrike => 3.0,
        AttackKind::PiercingShot => 4.0,
        AttackKind::HomingOrb => 5.0,
        AttackKind::Ricochet => 6.0,
        AttackKind::FrostShot => 7.0,
        AttackKind::ThunderBolt => 8.0,
        AttackKind::ShadowSneak => 9.0,
    }
}

fn f64_to_attack_kind(v: f64) -> AttackKind {
    match v as u32 {
        1 => AttackKind::PowerShot,
        2 => AttackKind::CurveBall,
        3 => AttackKind::MultiStrike,
        4 => AttackKind::PiercingShot,
        5 => AttackKind::HomingOrb,
        6 => AttackKind::Ricochet,
        7 => AttackKind::FrostShot,
        8 => AttackKind::ThunderBolt,
        9 => AttackKind::ShadowSneak,
        _ => AttackKind::Strike,
    }
}

fn do_instant_attack(engine: &mut Engine, attack: &Attack) {
    // ThunderBolt / Tremor — instant damage, no physics
    let active = gs(engine, K_ACTIVE_MON) as usize;
    ...
    let eff = type_effectiveness(attack.element, get_species(enemy_species).element);
    let variance = 0.85 + rng(engine) * 0.30;
    let raw_dmg = (atk * 1.5 - def * 0.5) * eff * attack.power * variance;
    ...
    ss(engine, K_DMG_POPUP_X, 15.0 * TILE_SIZE);
    ss(engine, K_DMG_POPUP_Y, 7.0 * TILE_SIZE);
    ss(engine, K_DMG_POPUP_TIMER, 1.0);
    ss(engine, K_DMG_CRIT, 0.0);
    ss(engine, K_ENEMY_SHAKE, 0.3);
    let strokes = gs(engine, K_STROKES);
    ss(engine, K_STROKES, strokes + 1.0);

    // Element sound
    play_element_hit_sound(engine, attack.element, false);
    engine.screen_fx.push(ScreenEffect::Flash {
        color: attack.element.color(), intensity: 0.4,
    }, 0.3);
...
fn handle_battle_items_tap(engine: &mut Engine, _x: f64, y: f64) {
    let items = [ITEM_POTION, ITEM_SUPER_POTION, ITEM_REVIVE];
    let start_y = 250.0;
    let item_h = 60.0;
    ...
    // Switch button
    if y > start_y + 3.0 * item_h && y < start_y + 3.0 * item_h + item_h {
        play_ui_click(engine);
        ss(engine, K_MODE, MODE_BATTLE_SWITCH);
        return;
    }
...
fn use_battle_item(engine: &mut Engine, item: u8) {
    let active = gs(engine, K_ACTIVE_MON) as usize;
    ...
    match item {
        ITEM_POTION => {
            ...
            if hp > 0.0 && hp < maxhp {
                ss_team(engine, active, "hp", (hp + 30.0).min(maxhp));
                ss_inv(engine, ITEM_POTION, gs_inv(engine, ITEM_POTION) - 1.0);
                play_heal_sound(engine);
            } else { return; }
        }
        ITEM_SUPER_POTION => {
            ...
            let maxhp = gs_team(engine, active, "maxhp");
            if hp > 0.0 && hp < maxhp {
                ss_team(engine, active, "hp", (hp + 60.0).min(maxhp));
                ss_inv(engine, ITEM_SUPER_POTION, gs_inv(engine, ITEM_SUPER_POTION) - 1.0);
                play_heal_sound(engine);
            } else { return; }
        }
        ITEM_REVIVE => {
            // Revive first fainted monster
            let team_size = gs(engine, K_TEAM_SIZE) as usize;
            let mut used = false;
            for i in 0..team_size {
            ...
            if !used { return; }
            ss_inv(engine, ITEM_REVIVE, gs_inv(engine, ITEM_REVIVE) - 1.0);
        }
        _ => return,
    }
...
fn handle_battle_switch_tap(engine: &mut Engine, _x: f64, y: f64) {
    let team_size = gs(engine, K_TEAM_SIZE) as usize;
    ...
    for i in 0..team_size.min(6) {
        let iy = start_y + i as f64 * slot_h;
        if y >= iy && y < iy + slot_h {
            if i != current && gs_team(engine, i, "hp") > 0.0 {
                ss(engine, K_ACTIVE_MON, i as f64);
                play_ui_click(engine);

                // Switching costs a turn in battle
                if gs(engine, K_MODE) == MODE_BATTLE_SWITCH {
                    ss(engine, K_MODE, MODE_BATTLE);
                    ss(engine, K_BPHASE, BPHASE_ENEMY_TURN);
                    ss(engine, K_BATTLE_TIMER, 0.0);
                    // Reset ball position for new monster
                    ss(engine, K_BALL_X, 15.0 * TILE_SIZE);
                    ss(engine, K_BALL_Y, 35.0 * TILE_SIZE);
                    ss(engine, K_BALL_VX, 0.0);
                    ss(engine, K_BALL_VY, 0.0);
                    ss(engine, K_SELECTED_ATTACK, 0.0);
                }
                return;
            }
        }
    ...
    if y > HEIGHT - 80.0 {
        play_ui_click(engine);
        let prev_mode = gs(engine, K_MODE);
        if prev_mode == MODE_BATTLE_SWITCH {
            ss(engine, K_MODE, MODE_BATTLE);
        } else {
            ss(engine, K_MODE, MODE_OVERWORLD);
        }
...
fn handle_starter_choice_tap(engine: &mut Engine, _x: f64, y: f64) {
    // Three starter buttons stacked vertically
    let choices: [(u8, &str); 3] = [(0, "Sproutail"), (2, "Emberpup"), (4, "Bubblefin")];
    let start_y = 280.0;
    let btn_h = 80.0;

    for (i, &(species_id, _name)) in choices.iter().enumerate() {
        let by = start_y + i as f64 * (btn_h + 10.0);
        if y >= by && y < by + btn_h {
            play_ui_click(engine);
            // Give chosen starter
            ss(engine, K_TEAM_SIZE, 1.0);
            set_team_monster(engine, 0, species_id, 5);
            ss(engine, K_STARTER_SEL, i as f64);
            ss(engine, K_STARTER_CONFIRMED, 1.0);

            // Transition to overworld
            ss(engine, K_MODE, MODE_DIALOGUE);
            ss(engine, K_DLG_ID, 20.0); // starter chosen dialogue
            ss(engine, K_DLG_LINE, 0.0);
            ss(engine, K_DLG_TIMER, 0.0);

            play_catch_sound(engine);
            engine.screen_fx.push(ScreenEffect::Flash {
                color: get_species(species_id).element.color(),
                intensity: 0.5,
            }, 0.3);
            return;
        }
...
fn try_catch_aim(engine: &mut Engine) {
    let orbs = gs_inv(engine, ITEM_SPIRIT_ORB);
    let ultra = gs_inv(engine, ITEM_ULTRA_ORB);
    if orbs <= 0.0 && ultra <= 0.0 {
        ss(engine, K_BATTLE_MSG, 7.0); // "No Spirit Orbs!"
        ss(engine, K_MSG_TIMER, 1.5);
        return;
    }

    if gs(engine, K_IS_BOSS) == 1.0 {
        ss(engine, K_BATTLE_MSG, 3.0);
        ss(engine, K_MSG_TIMER, 1.5);
        return;
    }

    play_ui_click(engine);
    // Enter catch aiming mode — same slingshot physics, but orb becomes a catch ball
    ss(engine, K_BPHASE, BPHASE_CATCH_AIM);
    ss(engine, K_BALL_X, 15.0 * TILE_SIZE);
    ss(engine, K_BALL_Y, 35.0 * TILE_SIZE);
    ss(engine, K_BALL_ACTIVE, 0.0);
}
...
pub fn update(engine: &mut Engine, dt: f64) {
    ...
    if mode == MODE_STARTER_CHOICE {
        return; // event-driven, no continuous update
    }

    // Shop, Heal, Menu, Battle Items/Switch: event-driven
...
fn update_battle(engine: &mut Engine, dt: f64) {
    ...
    if bphase == BPHASE_PLAYER_SHOT || bphase == BPHASE_CATCH_SHOT {
        update_ball_physics(engine, dt);
        return;
    }

    if bphase == BPHASE_CATCH_AIM {
        // Waiting for player to aim catch orb — no auto-advance
        return;
    }
...
fn update_ball_physics(engine: &mut Engine, dt: f64) {
    ...
    let atk_kind = f64_to_attack_kind(gs(engine, K_ATTACK_KIND));
    let is_catch = gs(engine, K_BPHASE) == BPHASE_CATCH_SHOT;
    ...
    // Attack-specific friction
    let friction = if is_catch { 0.98 } else {
        match atk_kind {
            AttackKind::PowerShot => 0.995,
            AttackKind::FrostShot => 0.99,
            AttackKind::Ricochet => 0.985,
            _ => 0.98,
        }
    };
    vx *= friction;
    ...
    // CurveBall: add perpendicular force
    if atk_kind == AttackKind::CurveBall && !is_catch {
        let speed = (vx * vx + vy * vy).sqrt();
        if speed > 20.0 {
            let curve_force = 200.0 * dt;
            // Perpendicular to velocity
            vx += (-vy / speed) * curve_force;
            vy += (vx / speed) * curve_force;
        }
    }

    // HomingOrb: slight steering toward target
    if atk_kind == AttackKind::HomingOrb && !is_catch {
        let target_cx = 15.0 * TILE_SIZE;
        let target_cy = 7.0 * TILE_SIZE;
        let dx = target_cx - bx;
        let dy = target_cy - by;
        let dist = (dx * dx + dy * dy).sqrt().max(1.0);
        let homing_str = 80.0 * dt;
        vx += (dx / dist) * homing_str;
        vy += (dy / dist) * homing_str;
    }

    // FrostShot: strong deceleration near target
    if atk_kind == AttackKind::FrostShot && !is_catch {
        let target_cx = 15.0 * TILE_SIZE;
        let target_cy = 7.0 * TILE_SIZE;
        let dx = bx - target_cx;
        let dy = by - target_cy;
    ...
    let wall_bounce = if is_catch { 0.8 } else {
        match atk_kind {
            AttackKind::Ricochet => 1.0, // perfect bounce
            AttackKind::FrostShot => 0.5, // dies on walls
            _ => 0.8,
        }
    };

    if bx < min_x { bx = min_x; vx = -vx * wall_bounce; play_wall_bounce(engine); let wb = gs(engine, K_WALL_BOUNCES); ss(engine, K_WALL_BOUNCES, wb + 1.0); }
    if bx > max_x { bx = max_x; vx = -vx * wall_bounce; play_wall_bounce(engine); let wb = gs(engine, K_WALL_BOUNCES); ss(engine, K_WALL_BOUNCES, wb + 1.0); }
    if by < min_y { by = min_y; vy = -vy * wall_bounce; play_wall_bounce(engine); let wb = gs(engine, K_WALL_BOUNCES); ss(engine, K_WALL_BOUNCES, wb + 1.0); }
    if by > max_y { by = max_y; vy = -vy * wall_bounce; play_wall_bounce(engine); let wb = gs(engine, K_WALL_BOUNCES); ss(engine, K_WALL_BOUNCES, wb + 1.0); }

    // Bumper collision (PiercingShot and ShadowSneak ignore bumpers)
    let ignore_bumpers = matches!(atk_kind, AttackKind::PiercingShot | AttackKind::ShadowSneak) && !is_catch;
    if !ignore_bumpers {
        if let Some(ref tm) = engine.tilemap {
    ...
    if dist < hit_radius {
        // Catch shot: physics-based catching
        if is_catch {
            do_physics_catch(engine, dist);
            return;
        }
        ...
        // Get attack element (may differ from species element)
        let sel_idx = gs(engine, K_SELECTED_ATTACK) as usize;
        let active_species = gs_team(engine, active, "species") as u8;
        let level = gs_team(engine, active, "level");
        let attacks = get_attacks(active_species, level);
        let attack = if sel_idx < attacks.len() { &attacks[sel_idx] } else { &attacks[0] };
        let atk_elem = attack.element;
        let attack_power = attack.power;
        let enemy_species = gs(engine, K_ENEMY_SPECIES) as u8;
        ...
        let raw_dmg = (atk * 2.0 - def * 0.8) * eff * speed_mult * crit_mult * variance * attack_power;
        let damage = raw_dmg.max(1.0).floor();
...
fn do_physics_catch(engine: &mut Engine, dist: f64) {
    // Physics-based catching: the catch orb HIT the enemy zone
    // Catch zone is bigger when enemy HP is lower
    let species_id = gs(engine, K_ENEMY_SPECIES) as u8;
    let sp = get_species(species_id);
    let hp = gs(engine, K_ENEMY_HP);
    let maxhp = gs(engine, K_ENEMY_MAXHP);
    let hp_ratio = (hp / maxhp).max(0.01);

    // Catch radius scales with damage done
    let damage_ratio = 1.0 - hp_ratio;
    let max_catch_radius = 2.5 * TILE_SIZE * (0.5 + sp.catch_rate);
    let catch_radius = max_catch_radius * (0.3 + 0.7 * damage_ratio);
    ...
    // Catch succeeds if ball is within the scaled catch radius
    let success = dist < catch_radius * catch_bonus;

    ss(engine, K_BALL_ACTIVE, 0.0);
    play_catch_sound(engine);
    ss(engine, K_MODE, MODE_CATCH_ANIM);
    ss(engine, K_CATCH_TIMER, 0.0);
    ss(engine, K_CATCH_SUCCESS, if success { 1.0 } else { 0.0 });

    if success {
        engine.screen_fx.push(ScreenEffect::Flash {
            color: Color { r: 200, g: 200, b: 255, a: 255 },
            intensity: 0.5,
        }, 0.3);
    }
}
